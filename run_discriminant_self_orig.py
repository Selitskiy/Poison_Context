...
from prompts import prompt_1, prompt_2, prompt_8
...
def discriminantSelfOrigFunct(row_num, row, mConf):
  ...
  if not response2 or not response2Num:
    prompt = prompt_8(haiku, response)
...
if __name__ == "__main__":
  ...
  for mConf in get_all_models_gen(): #get_open_models_gen(): #get_commercial_models_gen():
    ...
    experimentTpl = "discriminant_self_orig"
    newFields = ["response2Num","response2"]
    run_experiment(inputFileTpl, experimentTpl, newFields, discriminantSelfOrigFunct)
