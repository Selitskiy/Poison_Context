...
def run_analysis_hypot(fileTpl, fileTpl2, expTpl1, expTpl2, experimentTpl1, experimentTpl2, accuracyFunct):
  ...
  sample_metrics = accuracyFunct(dummy_df, dummy_df, expTpl1, expTpl2)
  ...
  for mConf1 in get_all_models_gen(): #get_open_models_gen(): #get_commercial_models_gen():
    ...
    for i, mConf2 in enumerate(get_all_models_gen()): #get_open_models_gen(): #get_commercial_models_gen():
      ...
      inputFileName1 = f"{fileTpl}_{expTpl1}_{modelName1}_{experimentTpl1}_{modelName2}.csv"
      inputFileFull1 = os.path.join(os.path.dirname(__file__), "data", inputFileName1)

      inputFileName2 = f"{fileTpl}_{expTpl2}_{modelName1}_{experimentTpl1}_{modelName2}.csv"
      inputFileFull2 = os.path.join(os.path.dirname(__file__), "data", inputFileName2)

      inputFileName3 = f"{fileTpl2}_{expTpl1}_{modelName1}_{experimentTpl2}_{modelName2}.csv"
      inputFileFull3 = os.path.join(os.path.dirname(__file__), "data", inputFileName3)

      inputFileName4 = f"{fileTpl2}_{expTpl2}_{modelName1}_{experimentTpl2}_{modelName2}.csv"
      inputFileFull4 = os.path.join(os.path.dirname(__file__), "data", inputFileName4)
      ...
      if not os.path.isfile(inputFileFull2):
        ...
        return(1)
      if not os.path.isfile(inputFileFull3):
        print(f"Input file 3 not found: {inputFileFull3}")
        return(1)
      if not os.path.isfile(inputFileFull4):
        print(f"Input file 4 not found: {inputFileFull4}")
        return(1)
      ...
      try:
        ...
        df3 = pd.read_csv(inputFileFull3)
        df3 = df3.rename(columns={"response2Num": expTpl1}) # rename to avoid conflict when merging
        df3 = df3[expTpl1] # get just the column we need

        df4 = pd.read_csv(inputFileFull4)
        ...
        exp3_list.append(df3)
        exp4_list.append(df4)
  ...
  print(f"Accuracy metrics for: {fileTpl}, {fileTpl2}, {expTpl1}, {expTpl2}, {experimentTpl1}, {experimentTpl2}")
  for name in metric_names:
    print(f"{name.capitalize()}:")
    print(df_metrics[name])

  print(f"Generator accuracy metrics for: {fileTpl}, {fileTpl2}, {expTpl1}, {expTpl2}, {experimentTpl1}, {experimentTpl2}")
  for name in metric_names:
    print(f"{name.capitalize()}:")
    print(dfr_metrics[name])

  print(f"Discriminator accuracy metrics for: {fileTpl}, {fileTpl2}, {expTpl1}, {expTpl2}, {experimentTpl1}, {experimentTpl2}")
  for name in metric_names:
    print(f"{name.capitalize()}:")
